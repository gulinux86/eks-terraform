resource "aws_subnet" "eks_subnet_public_1a" {
  ...
  # No kubernetes.io/role/elb tag: prevents the ALB Controller from creating
  # internet-facing load balancers. These subnets exist only to host the
  # NAT Gateways (egress for the private nodes).
  tags = merge(
    var.tags,
    {
